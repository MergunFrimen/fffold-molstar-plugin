...
export class SecondaryStructureSequence extends Sequence<SecondaryStructureSequenceProps> {
  protected updateMarker() {
    ...
    const secondarySpanDiv = this.parentDiv.current.querySelector(
      ".msp-sequence-secondary"
    );
    const emptySS =
      !!secondarySpanDiv &&
      /^[\s\u200b]+$/.test(secondarySpanDiv.textContent ?? "");
    const overlays = [" "];
  ...
  protected getSequenceNumberClass(
    ...
    const suffix = this.props.hideSecondaryStructure
      ? ""
      : " msp-sequence-number-above";
