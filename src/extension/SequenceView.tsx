  ...
  if (units.length > 0) {
    ...
    if (unit.polymerElements.length) {
      ...
      if (entitySeq && entitySeq.sequence.length <= MaxDisplaySequenceLength) {
        ...
        const polymerElementCount = units.reduce(
          (a, v) => a + v.polymerElements.length,
          0
        );
        if (
          Unit.isAtomic(unit) ||
          polymerElementCount > MaxDisplaySequenceLength
        ) {
      ...
      const residueCount = units.reduce(
        (a, v) => a + (v as Unit.Atomic).residueCount,
        0
      );
...
export function getModelEntityOptions(
  structure: Structure,
  polymersOnly = false
): [string, string][] {
